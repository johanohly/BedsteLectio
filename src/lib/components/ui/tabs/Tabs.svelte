<script lang="ts">
  ...
  const onEnterTab = (e: FocusEvent | PointerEvent, i: number) => {
    ...
    // Calculate the scroll position and set the hoveredRect
    const rect = e.target.getBoundingClientRect();
    const scrollY = window.scrollY;
    hoveredRect.set(new DOMRect(rect.x, rect.y + scrollY, rect.width, rect.height));
  };
  ...
  $: if (navRect && selectedRect) {
    ...
    selectStyles += "transition: " + (isInitialRender ? `opacity 150ms 150ms;` : `transform 150ms 0ms, opacity 150ms 150ms, width 150ms;`);

    isInitialRender = false;
  }
</script>

<nav bind:this={navRef} class="flex flex-shrink-0 items-center relative z-0 py-2" on:pointerleave={onLeaveTabs}>
  {#each tabs as tab, i}
    <button bind:this={$buttonRefs[i]} class="text-md relative rounded-md flex items-center h-8 px-4 z-20 bg-transparent text-base text-slate-500 dark:text-slate-400 cursor-pointer select-none transition-colors {$hoveredTabIndex === i || $selectedTabIndex === i ? '!text-black dark:!text-white' : ''}" id={String(i)} on:click={() => onSelectTab(i)} on:focus={(e) => onEnterTab(e, i)} on:pointerenter={(e) => onEnterTab(e, i)}>
      {tab.label}
    </button>
  {/each}
  <div class="hidden lg:block absolute z-10 top-0 left-0 rounded-md bg-gray-200 dark:bg-dark-hover transition-[width]" style={hoverStyles} />
  <div class="absolute z-10 bottom-0 left-0 h-0.5 bg-black dark:bg-white" style={selectStyles} />
</nav>
